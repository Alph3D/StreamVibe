...
const SinglePageLayout = ({ children, data, type }) => {
    // On ajoute thumbnail dans la déstructuration de data ici :
    const { _id: id, title, description, cover, thumbnail, language, genres, director, release_date, imdb_rating, rotten_rating } = data;

    return (
        <main className="container py-6">
            {/* On passe maintenant le thumbnail au composant TopHeader */}
            <TopHeader 
                id={id} 
                title={title} 
