...
// Pense à vérifier le nom exact de ta fonction de service ici si nécessaire
...
const PopularMoviesSection = () => {
    ...
    useEffect(() => {
        const getMovies = async () => {
            // Sécurisation contre les réponses d'API vides ou mal formées
    ...
    const handleNext = () => {
        if (scrollContainerRef.current) {
            ...
            setCurrentIndex((prevIndex) => Math.min(prevIndex + 1, (movies?.length || 1) - 1));
        }
    };
            ...
            >
                {loading
                    ...
                    movies?.length === 0 ? <span className="3xl:text-super-base xl:text-super-sm max-md:text-sm text-c-grey-60">Sorry, no movies available yet. Please visit us again later.</span>
                        // Correction ici : ajout de l'optional chaining (movies?.map)
