...
const TopMovieSection = () => {
    ...
    const handleNext = () => {
        if (scrollContainerRef.current) {
            ...
            setCurrentIndex((prevIndex) => Math.min(prevIndex + 1, Object.keys(categories || {}).length - 1));
        }
    };
    ...
    // Nettoie le tableau d'images pour extraire uniquement les chaînes de caractères (URLs)
    const cleanThumbnails = (rawImages) => {
        if (!Array.isArray(rawImages)) return [];
        return rawImages.map(img => {
            if (typeof img === 'string') return img;
            if (img && typeof img === 'object') {
                return img.url || img.image || img.thumbnail || img.poster_path || img.backdrop_path || '';
            }
            return '';
                ...
                {loading || categoryEntries.length === 0
                    ...
                    : categoryEntries.map(([category, thumbnail], index) => (
                        <MultipleCard 
                            key={index} 
                            title={category} 
                            images={cleanThumbnails(thumbnail)} // <-- Injection du tableau d'images nettoyé
                            baseurl={"/series/genres"} 
                            topRated 
                        />
                    ))}
            </div>
